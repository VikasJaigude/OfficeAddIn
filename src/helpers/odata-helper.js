...
const deleteDraft = function(accessToken, 
    domain, 
    apiURLsegment, 
    apiVersion, 
    ...
    return new Promise((resolve, reject) => {
        ...
        console.log(`inside deleteDraft ${JSON.stringify(options)}`);
        https.request(options, function (response) {
            ...
            response.on('end', function () {
                ...
                if (response.statusCode === 200) {
                    ...
                    console.log(`Error  Code :=> ${JSON.parse(body).error.code} || Message :=> ${JSON.parse(body).error.message}`);
                    resolve(error);
                }
            });
        })
...
const callGraphApi = function(accessToken, 
    url, 
    itemId) {
